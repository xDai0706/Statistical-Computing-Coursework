...
def get_word_score(word, n):
    ...
    letter_scores = sum(SCRABBLE_LETTER_VALUES.get(letter, 0) for letter in word)
    second_component = 7 * word_length - 3 * (n - word_length)
    second_component = max(second_component, 1)
    return letter_scores * second_component
...
def substitute_hand(hand, letter):
    ...
    used_letters = "".join(hand.keys())
    unused_letters = ""
    unused_letters = "".join([letter for letter in all_letters if letter not in used_letters])
    new_letter = random.choice([l for l in unused_letters if l != letter])
    new_hand = hand.copy()
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(n_hands):
        ...
        if substitute_used == False:
            substitute_choice = input('Would you like to substitute a letter? ').lower()
        ...
        if replay_used == False:
            replay_choice = input('Would you like to replay the hand? ').lower()
